...
class SkinDiseaseModel(nn.Module):
    def __init__(self):
        ...
        self.disease_head = nn.Linear(in_features, CONFIG.n_classes)
...
RISK_LOSS_WEIGHT = 0.5  # 보조 task 가중치 (의학 논문 표준 범위)
...
def comprehensive_evaluation(model, data_loader, dataset_name="Test"):
    ...
    cm_path = os.path.join(CONFIG.project_root, f"{dataset_name.lower()}_confusion_matrix.csv")
    ...
    try:
        ...
        cm_img_path = os.path.join(CONFIG.project_root, f"{dataset_name.lower()}_confusion_matrix.png")
...
def visualize_model_performance(history, results=None, save_dir=None):
    ...
    if results is not None:
        ...
        if 'risk_sensitivities' in results and 'risk_fnrs' in results:
            ...
            plt.tight_layout()
            save_path = os.path.join(save_dir, 'risk_level_performance.png')
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    if RUN_TEST:
        ...
        visualize_model_performance(
            ...
            save_dir=os.path.join(CONFIG.project_root, "test_results")
        )
    ...
    history_path = os.path.join(CONFIG.project_root, "training_history.csv")
